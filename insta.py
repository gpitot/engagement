...
class User:
	...
	def get_recent(self):
		self.recent_photos = [node["thumbnail_src"] for node in self.page_json["user"]["media"]["nodes"][:3]]
		self.recent_photos_urls = [node["code"] for node in self.page_json["user"]["media"]["nodes"][:3]]
